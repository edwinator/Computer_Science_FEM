...
const findMostCommonTitle = (myId, degreesOfSeparation) => {
  ...
  for (let i = 0; i <= degreesOfSeparation; i++) {
    ...
    while (queue.length) {
      ...
      for (let j = 0; j < user.connections.length; j++) {
        const thisUser = user.connections[j];
        if (!seen.has(thisUser)) {
          thisQueue.push(thisUser);
          seen.add(thisUser);
        }
      }
  ...
  let jobName = keys[0];
  let biggestJobCount = jobs[keys[0]];
  for (let i = 0; i < keys.length; i++) {
    const currJob = keys[i];
    if (jobs[currJob] > biggestJobCount) {
      jobName = currJob;
      biggestJobCount = jobs[currJob];
    }
  }
  return jobName;
};
